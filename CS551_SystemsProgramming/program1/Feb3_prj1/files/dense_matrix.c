 ...
 */
inline static void checkMatrixState( const  struct DenseImplMatrixStruct *pConstDenseMatrix, int *pErr ){
...
static MatrixBaseType getElement( const Matrix *pConstMatrix, int rowIndex, int colIndex,  int *pErr){
	...
	MatrixBaseType thisElement = pConstDenseMatrix->pData[ (rowIndex * (pConstDenseMatrix->nCols)) + colIndex] ;
	return thisElement;
}

static void setElement( Matrix *pMatrix, int rowIndex, int colIndex, MatrixBaseType element, int *pErr){
	struct DenseImplMatrixStruct *pDenseMatrix = (struct DenseImplMatrixStruct *) pMatrix;
	checkMatrixState( pDenseMatrix, pErr );
	checkPassedRowAndCol( pDenseMatrix, rowIndex, colIndex, pErr);
	pDenseMatrix->pData[ rowIndex * pDenseMatrix->nCols + colIndex] = element;
}
...
newDenseMatrix(int nRows, int nCols, int *pErr)
{
  ...
  struct DenseImplMatrixStruct *pDenseImplMatrix = calloc( 1, ( sizeof(struct DenseImplMatrixStruct) + numElements*sizeof(MatrixBaseType)));
  if( !pDenseImplMatrix ){
	  *pErr = ENOMEM;
	  fprintf(stderr, "newDenseMatrix: DenseImplMatrixStruct memory allocation failure.\n");
	  exit(1);
  }
  pDenseImplMatrix->nRows = nRows;
  pDenseImplMatrix->nCols = nCols;
  ...
  pDenseImplMatrix->fns = (MatrixFns *)&denseMatrixFns;
  
  return ((DenseMatrix *) pDenseImplMatrix);
}
